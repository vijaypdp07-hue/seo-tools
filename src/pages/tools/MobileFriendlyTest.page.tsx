...
import { Search, Globe, Smartphone, CheckCircle, XCircle } from "lucide-react";
import { ToolWrapper } from "@/components/shared/ToolWrapper";
import { toast } from "sonner";
import { clsx } from "clsx";

export function MobileFriendlyTestPage() {
  ...
  const handleCheck = async () => {
    ...
    try {
      const response = await fetch(`/api/tools/mobile-friendly?url=${encodeURIComponent(url)}`);
      if (!response.ok) {
      ...
      setResult(data);
      toast.success("Mobile friendly test completed!");
    } catch (error: any) {
      console.error(error);
          ...
          <div className="p-6 md:p-12 space-y-8 max-w-4xl mx-auto w-full">
              ...
              <div className="bg-bg-secondary p-8 rounded-xl border border-border-base shadow-sm space-y-4">
                  <div className="space-y-1">
                      ...
                      <p className="text-sm text-text-secondary">Enter the URL to check its mobile friendliness.</p>
                  </div>
              ...
              {result && (
                  <div className={clsx(
                    ...
                    result.isMobileFriendly ? "border-success bg-success-light/20" : "border-warning bg-warning-light/20"
                  )}>
                      {result.isMobileFriendly ? (
                          ...
                          <div className="size-16 rounded-full bg-warning/10 flex items-center justify-center text-warning mb-2">
                             <XCircle className="size-8" />
                          </div>
                      )}
                      
                      <h4 className="text-2xl font-bold text-text-primary">
                          {result.isMobileFriendly ? "Page is Mobile Friendly" : "Page is Not Mobile Friendly"}
                      </h4>
                      ...
                      <div className="flex flex-wrap gap-4 mt-6 justify-center w-full max-w-md">
                          <div className="bg-bg-base border border-border-base rounded p-4 flex-1">
                             <div className="text-sm text-text-muted uppercase tracking-wider font-bold mb-1">Mobile SEO Score</div>
                             <div className="text-3xl font-black text-text-primary">{result.seoScore}</div>
                          </div>
                          <div className="bg-bg-base border border-border-base rounded p-4 flex-1">
                             <div className="text-sm text-text-muted uppercase tracking-wider font-bold mb-1">Performance</div>
                             <div className="text-3xl font-black text-text-primary">{result.performanceScore}</div>
                          </div>
                      </div>
                  </div>
              )}
